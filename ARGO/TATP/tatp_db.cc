...
void TATP_DB::initialize(unsigned num_subscribers, int n) {
  ...
  num_threads = n;
  subscriber_table = (subscriber_entry*) malloc(num_subscribers*sizeof(subscriber_entry));

  // A max of 4 access info entries per subscriber
  access_info_table = (access_info_entry*)malloc(4*num_subscribers*sizeof(access_info_entry));

  // A max of 4 access info entries per subscriber
  special_facility_table = (special_facility_entry*) malloc(4*num_subscribers*sizeof(special_facility_entry));

  // A max of 3 call forwarding entries per "special facility entry"
  call_forwarding_table= (call_forwarding_entry*) malloc(3*4*num_subscribers*sizeof(call_forwarding_entry));
